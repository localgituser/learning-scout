...
find relevant conferences, courses, cohort programs, or books.

Return ONLY a JSON array (no markdown, no preamble) of objects with these fields:
- title (string, required)
- url (string, required)
- description (string, required)
- category (string, required: one of in_person_events | online_courses | cohort_programs | books_or_resources | wildcard)
...
async def search_topic(
    ...
    combined = "\n".join(text_parts)
    return parse_items_from_response(combined)
...
async def run_search(
    ...
    tasks = [search_topic(t, config, client) for t in topics]
